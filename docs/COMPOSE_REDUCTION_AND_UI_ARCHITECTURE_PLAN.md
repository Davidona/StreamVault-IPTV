...
This branch was synchronized with the current local `develop` in merge commit `c2a2658f`. The merge was conflict-free and included the `1.0.18` changelog, grouped VOD performance/pagination work, dashboard shelf updates, orientation handling, and data-maintenance changes; none overlapped the Phase 1 extraction boundaries, so no compatibility code changes were required. The merged tree passes `:app:testDebugUnitTest` and `:app:assembleDebug`. `:data:testDebugUnitTest` runs 1,038 tests but retains the pre-existing `ProviderExecutionArchitectureTest` failure at `data/src/test/java/com/streamvault/data/provider/ProviderExecutionArchitectureTest.kt:276` (expected dependency budget 40, actual 42); `SyncManager` was not part of the merge.
