...
state/action callbacks. Focus reconciliation now resolves stale channel/program
selection and current-program fallback behind a feature-owned pure contract.
The EPG content host now owns loading, message, preview,
toolbar, refresh, and grid composition while the app supplies the localized
label bundles, content message, state, and action callbacks. The options
overlay and EPG-match dialog likewise own their presentation and focus behavior
...
source-category protection behind feature-owned pure predicates; `EpgScreen`
keeps its compatibility wrappers while supplying current state.
The EPG category-selection fallback policy now preserves the existing
All/Favorites/accessibility ordering and explicit-unlock behavior behind a
feature-owned pure resolver; `EpgViewModel` delegates while retaining repository
coordination.
Guide schedule metrics (missing entries, channels with schedules, and upcoming
